...
import { getAttendancesPerUser } from "../../events";
import { Columns, SheetData } from "write-excel-file";
import { getSavedNeededStudyTimes } from "../../studyTime";
...
async function getAttendedEventsXLSX(user: User, cw: number, year: number) {
    ...
    attendances.forEach((attendance) => {
        ...
        {
            "type": Date,
            "value": new Date(attendance.event.createdAt),
            "format": "DD.MM.YYYY HH:mm"
        },
        {
            "type": Date,
            "value": new Date(attendance.attendance.createdAt),
            "format": "DD.MM.YYYY HH:mm"
        }])
    })
    for (let i = 0; i < 8; i++) {
