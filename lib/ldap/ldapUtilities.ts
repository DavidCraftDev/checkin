...
import { existsTeacherCompetenceFile, getTeacherCompetenceFile } from '@/lib/data/competences';
...
async function updateUserData(ldapData: Entry[]) {
    ...
    if (createData.length > 0) {
        await db.user.createMany({ data: createData, skipDuplicates: true })
    }
}
...
function readLDAPUserData(ldapUser: Entry, dbUser?: User) {
    ...
    let groupsList: Array<string> = new Array<string>
    if (config_data.LDAP.AUTOMATIC_DATA_DETECTION.GROUPS.ENABLE) {
        ldapUser.memberOf.map((groupData: string) => {
            ...
            if (data[1].toLowerCase() == config_data.LDAP.AUTOMATIC_DATA_DETECTION.GROUPS.GROUP_OU.toLowerCase()) groupsList.push(String(data[0].replace("CN=", "").replace("cn=", "")))
        })
    }
    ...
    const groupData = { groups: groupsList }
