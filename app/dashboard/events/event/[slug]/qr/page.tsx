...
async function QRScannerPage({ params }: { params: Promise<{ slug: string }> }) {
    const user = await getSessionUser(1);
    const event = await getEventPerID((await params).slug);
    ...
    return (
        <div>
            <div className="grid grid-rows-1 grid-cols-1 md:grid-cols-2 mb-4">
                ...
                <a className="btn w-max h-min place-self-center items-center mt-2 md:mt-0" href={`/dashboard/events/event/${event.id}`}>Zurück zur Studienzeit</a>
            </div>
