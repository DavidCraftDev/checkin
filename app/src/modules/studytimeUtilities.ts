...
export async function getNeededStudyTimesSelect(userID: string, teacherID: string) {
  ...
  const attendances = await getAttendancesPerUser(userID, moment().isoWeekYear(), moment().year());
...
export async function getMissingStudyTimes(userID: string) {
  ...
  const attendedStudyTimes = await getAttendedStudyTimes(userID, moment().isoWeekYear(), moment().year());
