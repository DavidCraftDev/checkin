...
FROM --platform="$BUILDPLATFORM" node:22.13.0-alpine3.21 AS build
...
#      rm -rv /app/node_modules/@next/swc-linux-*-gnu && \
      rm -v /app/node_modules/@prisma/engines/schema-engine-linux-musl-openssl-* && \
      rm -v /app/node_modules/prisma/libquery_engine-linux-musl-openssl-*.so.node && \
      rm -v /app/node_modules/.prisma/client/libquery_engine-linux-musl-openssl-*.so.node && \
...
#      rm -rvf /app/node_modules/@next/swc-linux-*-gnu && \
      rm -rv /app/node_modules/@prisma/engines/schema-engine-linux-musl-openssl-* && \
      rm -v /app/node_modules/prisma/libquery_engine-linux-musl-arm64-openssl-*.so.node && \
      rm -v /app/node_modules/.prisma/client/libquery_engine-linux-musl-arm64-openssl-*.so.node && \
