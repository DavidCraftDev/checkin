  ...
  return (
    <>
      {links.map((link) => {
        ...
        if (link.name === "Meine Gruppe" && (props.groups.length < 1 || props.groups[0] === "")) return null;
        if (!props.untisEnabled && link.name === "Stundenplan") return null;

        const label = link.name === "Meine Gruppe" && props.groups.length > 1 ? "Meine Gruppen" : link.name;
        const href = link.name === "Meine Gruppe" && props.groups.length > 1 ? "/dashboard/groups/mygroups" : link.href;
